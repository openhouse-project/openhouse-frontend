<script>
    import { makeChainStore, defaultChainStore, web3, connected, selectedAccount, chainId, chainData } from 'svelte-web3'
  ...
  {#if $web3.version}
  <div class="buttons">
    <button class="btn btn-primary" on:click="{enableBrowser}">connect to the browser window provider </button> (eg Metamask)
  </div>
  {/if}
